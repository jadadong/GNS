...
class SAGE(nn.Module):
    ...
    def forward(self, blocks, x,A=None):
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            h = layer(block, h)
            if self.buffer_size !=0 and self.IS==1 and l == 0:
...
def run(args, device, data):
    ...
    model = SAGE(in_feats, args.num_hidden, n_classes, args.num_layers, F.relu, args.dropout,
                 [int(fanout) for fanout in args.fan_out.split(',')], args.buffer_size,args.batch_size/number_of_nodes,args.IS)
...
#                dst_id = block[1].dstdata.__getitem__('_ID')
#
#                dst_in_dgrees.append(in_degree_all_nodes[dst_id])
...
#    data = load_data(args)
    print('loading graph')
    adj_full, adj_train, feats, class_map, role=load_data('amazon')
