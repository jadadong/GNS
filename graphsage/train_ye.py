    ...
    for epoch in range(args.num_epochs):
        if epoch % args.buffer_rs_every ==0:
            if args.buffer_size != 0:
                ...
                num_sample_nodes = int(args.buffer_size * num_nodes)  # a parameter that to tune
 #            args.buffer = np.random.permutation(num_nodes)[:num_sample_nodes]
                args.buffer =np.random.choice(num_nodes, num_sample_nodes, replace=False, p=np.divide(in_degree_all_nodes, sum(in_degree_all_nodes)))
            sampler = dgl.dataloading.MultiLayerNeighborSampler(
                [int(fanout) for fanout in args.fan_out.split(',')], args.buffer, args.buffer_size,g)
            dataloader = dgl.dataloading.NodeDataLoader(
                train_g,
                train_nid,
                sampler,
