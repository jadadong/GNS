...
def weighted_spmm(block, inputs, in_degree_all_nodes, cached_in_degree, prob, cache_size, max_fanout):
    src, dst = block.edges()
    ...
    sample_prob = 1 - th.pow(1 - prob[src], cache_size)
    prob2 = max_fanout / th.maximum(cached_N,
                                    th.ones(len(cached_N), device=cached_N.device) * max_fanout)
    sample_prob = sample_prob * prob2

    block.edata['prob'] = (1 / (sample_prob * N)).float()
    msg_fn = fn.u_mul_e('h', 'prob', 'm')
    block.srcdata['h'] = inputs
    reduce_fn = fn.sum('m', 'neigh')
    block.update_all(msg_fn, reduce_fn)
    #print('0-degree:', th.sum(block.in_degrees() == 0), len(block.in_degrees()))
    #print(block.dstdata['neigh'][block.in_degrees() == 0])
    return block.dstdata['neigh']

def unweighted_spmm(block, inputs):
...
def run(args, device, data):
    ...
    if args.buffer_size != 0:
        min_fanout[0] = 0
        max_fanout[0] = g.number_of_nodes()
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            ...
            h1 = weighted_spmm(blocks[0], batch_inputs, in_degree_all_nodes, cached_in_degree, prob_gpu, num_sample_nodes, max_fanout[0])
            h2 = unweighted_spmm(blocks[0], batch_inputs)
            block_in_degree = blocks[0].in_degrees()
            print('0-degree:', th.sum(block_in_degree == 0), len(block_in_degree))
            in_subgraph = g.in_subgraph(blocks[0].dstdata[dgl.NID].cpu())
            in_subgraph = dgl.to_block(in_subgraph)
            print('0-degree:', th.sum(in_subgraph.in_degrees() == 0), len(in_subgraph.in_degrees()))
            subgraph_inputs = g.ndata['feat'][in_subgraph.srcdata[dgl.NID].cpu()]
            h3 = unweighted_spmm(in_subgraph, subgraph_inputs)
            sort_idx = th.argsort(blocks[0].dstdata[dgl.NID])
            block_in_degre = block_in_degree[sort_idx]
            assert np.all(blocks[0].dstdata[dgl.NID][sort_idx].cpu().numpy() == in_subgraph.dstdata[dgl.NID].numpy())
            print(th.mean(th.abs(h1)), th.mean(th.abs(h2)), th.mean(th.abs(h3)))
            print('weighted - full:', th.mean(th.abs(h1[sort_idx][block_in_degree > 0].cpu() - h3[block_in_degree > 0])))
            print('unweighted - full:', th.mean(th.abs(h2[sort_idx][block_in_degree > 0].cpu() - h3[block_in_degree > 0])))
